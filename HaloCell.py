    ...
    for step in range(NSTEPS):
        ...
        if step % 100 == 0:
            ...
            if rank == 0:
                ...
                # Streamplot
                axs[0].streamplot(X, Y, u_x.T, u_y.T, density=1.2, linewidth=1, arrowsize=1.5)
                axs[0].set_xlim(xlim)
                axs[0].set_ylim(ylim)
                axs[0].set_title(f'Lid-driven cavity flow (Steps:{step:05d})')
                axs[0].set_xlabel("X")
                axs[0].set_ylabel("Y")
                axs[0].axis("equal")
                axs[0].grid(True)
                # 2. Speed Magnitude Heatmap
                speed = np.sqrt(u_x**2 + u_y**2)
                im1 = axs[1].imshow(speed.T, origin='lower', cmap='viridis')
